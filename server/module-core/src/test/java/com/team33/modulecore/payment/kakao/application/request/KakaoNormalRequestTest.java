...
class KakaoNormalRequestTest {
	...
	void 단건_결제_요청() throws Exception {
		...
		KakaoNormalRequest kakaoNormalRequest =
			new KakaoNormalRequest(
				(params, url) -> new KakaoRequestResponse(),
				parameterProvider
			);

		//when
		KakaoRequestResponse kakaoRequestResponse = kakaoNormalRequest.requestOneTime(order);
