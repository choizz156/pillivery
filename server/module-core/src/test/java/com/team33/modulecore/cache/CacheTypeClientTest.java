...
class CacheTypeClientTest {
	...
	void 할인_캐싱_x() throws Exception {
		...
		CacheClient2 cacheClient = new CacheClient2(redissonClient, itemQueryRepository);
		...
		RMapCache<String, CachedItems> cachedMainItems = redissonClient.getMapCache(CACHE_MAIN_ITEMS.name());
		CachedItems cachedMainItem = cachedMainItems.get(MAIN_DISCOUNT_ITEM.name());
	...
	void 할인_캐싱_o() throws Exception {
		...
		// when
		CachedItems mainItems = cacheClient.getMainDiscountItem();
	...
	void 카테고리_캐싱_x() throws Exception {
		...
		List<ItemQueryDto> content = categoryItems.getContent().getCachedItems();
	...
	void 아이템_조회수() throws Exception {
		...
		CacheClient2 cacheClient = new CacheClient2(redissonClient, null);
	...
	void 판매_캐싱_x() throws Exception {
		...
		RMapCache<String, CachedItems> cachedMainItems = redissonClient.getMapCache(CACHE_MAIN_ITEMS.name());
		CachedItems cachedMainItem = cachedMainItems.get(MAIN_SALES_ITEM.name());
