...
class ItemQueryDslTest {
	...
	void 세일_아이템_조회() throws Exception {
		...
		//when
		Page<ItemQueryDto> itemsOnSale = itemQueryRepository.findItemsOnSale(
			null,
			new PriceFilterDto(),
	...
	void 카테고리_조회() throws Exception {
		...
		Page<ItemQueryDto> itemsByCategory = itemQueryRepository.findItemsByCategory(
			CategoryName.BONE, null, new PriceFilterDto(), ItemPageDto.from(dto)
		);
	...
	private void persistItems() {
		...
		List<Item> items =  FixtureMonkeyFactory.get().giveMeBuilder(Item.class)
			...
			.set("categories", null)
			.set("includedCategories", Set.of(EYE))
			.sampleList(15);
		...
		var items2 =  FixtureMonkeyFactory.get().giveMeBuilder(Item.class)
			...
			.set("includedCategories", Set.of(BONE))
			.set("categories", null)
