...
@DisplayName("3번째 결제 재시도")
class RetryStepConfig3Test extends BatchTest {
	...
	@Autowired
	@Qualifier("delayedItemReader3")
	private ItemReader<DelayedItem> delayedItemReader3;

	@Autowired
	@Qualifier("delayedItemWriter3")
	private ItemWriter<DelayedItem> delayedItemWriter3;
	...
	void test1() throws Exception {
		...
		JpaPagingItemReader<DelayedItem> reader = (JpaPagingItemReader<DelayedItem>)delayedItemReader3;
	...
	void test3() throws Exception {
		...
		//when
		delayedItemWriter3.write(list);

		//then
		boolean result1 = delayedItemRepository.existsDelayedItemBySubscriptionOrderIdAndStatus(1L, ErrorStatus.CANCELLED);
		assertThat(result1).isTrue();
		boolean result2 = delayedItemRepository.existsDelayedItemBySubscriptionOrderIdAndStatus(2L, ErrorStatus.SOLVED);
