...
class PaymentApiDispatcherTest {
	...
	void test2() {
		...
		verify(mockRestTemplateSender, times(1)).sendToPost(anyLong(), urlCaptor.capture(), eq(null));
	}
	...
	void test3() {
		...
		ArgumentCaptor<String> urlCaptor = ArgumentCaptor.forClass(String.class);
		verify(mockRestTemplateSender, times(2)).sendToPost(anyLong(), urlCaptor.capture(), eq(null));
	}
    ...
    void test4() {
        ...
        // then
        verify(mockRestTemplateSender, times(1)).sendToPost(anyLong(), anyString(), eq(null));
    }
