...
class PaymentApiDispatcherTest {
	...
	void test2() {
		...
		OrderVO mockOrderVO = mock(OrderVO.class);
		when(mockOrderVO.getOrderId()).thenReturn(100L);
		...
		assertThat(urlCaptor.getValue()).isEqualTo(URL + mockOrderVO.getOrderId());
	}
	...
	void testDispatch_WhenListHasMultipleOrders_ExecutesSendForEachOrder() {
		...
		OrderVO mockOrderVO1 = mock(OrderVO.class);
		OrderVO mockOrderVO2 = mock(OrderVO.class);

		when(mockOrderVO1.getOrderId()).thenReturn(101L);
		when(mockOrderVO2.getOrderId()).thenReturn(102L);
		...
		assertThat(capturedUrls.get(0)).isEqualTo(URL + mockOrderVO1.getOrderId());
		assertThat(capturedUrls.get(1)).isEqualTo(URL + mockOrderVO2.getOrderId());
	}
}