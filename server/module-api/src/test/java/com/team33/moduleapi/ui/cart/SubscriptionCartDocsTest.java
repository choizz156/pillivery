...
class SubscriptionCartDocsTest extends WebRestDocsSupport {
    ...
    void 구독_카트_조회_문서화() throws Exception {
        given(spec)
    ...
    @Test
    @UserAccount({"test", "010-0000-0000"})
    void 구독_카트_상품_추가_문서화() throws Exception {
        ...
        given(spec)
            ...
            .filter(document("add-subscription-cart-item",
                ...
                requestFields(
                    ...
                    fieldWithPath("subscription").type(JsonFieldType.BOOLEAN).description("구독 상태"),
    ...
    @Test
    @UserAccount({"test", "010-0000-0000"})
    void 구독_카트_상품_삭제_문서화() throws Exception {
        given(spec)
            .header("Authorization", getToken())
            .param("itemId", 1)
            .log().all()
    ...
    @Test
    @UserAccount({"test", "010-0000-0000"})
    void 구독_카트_상품_수량_변경_문서화() throws Exception {
        given(spec)
            .header("Authorization", getToken())
            .param("quantity", 2)
            .param("itemId", 1)
    ...
    @Test
    @UserAccount({"test", "010-0000-0000"})
    void 구독_카트_상품_주기_변경_문서화() throws Exception {
        given(spec)
            .header("Authorization", getToken())
            .param("period", 60)
            .param("itemId", 1)
